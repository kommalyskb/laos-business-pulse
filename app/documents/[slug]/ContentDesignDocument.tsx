...
const specs: Record<string, DocumentSpec> = {
  ...
  "full-ux-ui": {
    ...
    version: "0.9.2", status: "pending", statusLabel: "Guest Prototype R2.3 + Admin Workflow Prototype R3.2 ສຳເລັດ · ລໍ Backend/Auth/Failure-state/Manual Gates ກ່ອນ 1.0",
    approvalNote: "ສະບັບ 0.9.2 ປັບ Navigation Contract ຂອງ Admin Workflow Prototype R3.2 ໃຫ້ຖືກຕ້ອງ: 10 Main Modules—Executive Overview, Operations Queue, Place 360, Content & Sources, Trust & Safety, Partner CRM, Sponsored Campaigns, Analytics, Revenue & Finance ແລະ System & Access—ເລືອກຜ່ານ Application Launcher ເທົ່ານັ້ນ. Header Menu ຖືກໃຊ້ສະເພາະ Sub-Menu ຂອງ Module ປັດຈຸບັນ; ຫ້າມສະແດງ Main Modules ຢູ່ Header ແລະຫ້າມສ້າງ Sub-Menu row ອີກຊັ້ນ. Brand context ສະແດງ Module ປັດຈຸບັນ ແລະ URL ເກັບ module/view. ໜ້າວຽກຂອງ 55 Sub-Menu views ຍັງຄົງມີ KPI, filterable register, selected-record context, owner, due date, business control, required evidence, checklist, activity/audit ແລະ next-best action. Data ແລະ Audit ເປັນ session fixtures ເທົ່ານັ້ນ; ຍັງບໍ່ມີ backend, authentication, durable persistence ຫຼື production permission enforcement. UX-05 ຍັງບໍ່ແມ່ນ 1.0 ຈົນ TEC-01/02, exhaustive failure states, Manual Accessibility Gates ແລະ final four-role sign-off ສຳເລັດ.",
    ...
    sections: [
      { title: "Final Design Baseline R1 · ສິ່ງທີ່ສຳເລັດແລ້ວ", intro: "Final Design Gallery ແມ່ນ Web Documentation ສຳລັບກວດໜ້າຕາ, hierarchy, responsive direction ແລະ failure state. ມັນບໍ່ແມ່ນ Production application ແລະບໍ່ສ້າງ business logic ແຍກຈາກ PRO-02/04.", headers: ["Deliverable", "ຂອບເຂດ", "ວິທີກວດ", "ສະຖານະ"], rows: [
        ["Final Design Gallery 0.9.2", "SCR-G01/G02/G03/G05 ໃຊ້ Guest Prototype R2.3; SCR-A00—A09 embed Admin Workflow Prototype R3.2 ໂດຍກົງ", "Admin ກວດ Application Launcher → Module → Header Sub-Menu → detailed workspace ແລະ audit", "Interactive baseline ready; backend/auth pending"],
        ["Admin Workflow Prototype R3.2", "Standalone /admin-prototype ພ້ອມ 10 Main Modules ໃນ Application Launcher ແລະ 55 Sub-Menu views ຢູ່ Header Menu", "ລອງ Application Launcher → Module; ກວດວ່າ Header ສະແດງສະເພາະ Sub-Menu ແລະບໍ່ມີ navigation row ຊັ້ນທີສອງ", "High-fidelity interaction prototype ready"],
        ["Single-level Header Navigation", "Main Modules ບໍ່ສະແດງໃນ Header; Header Menu ສະແດງ Sub-Menu ຂອງ active Module ໂດຍກົງ", "ປ່ຽນ Module ຜ່ານ Application Launcher ແລ້ວກວດ Sub-Menu context ແລະ URL module/view", "R3.2 baseline ready"],
      ...
      { title: "Approval Gates ແລະ Sign-off", intro: "UX-05 1.0 ແມ່ນຄຳຢືນຢັນວ່າ package ພ້ອມໃຫ້ Developer ສ້າງໂດຍບໍ່ເດົາ. ການມີຮູບ Happy path ສວຍງາມບໍ່ພຽງພໍ.", headers: ["Gate", "ຕ້ອງມີ", "Signer", "ສະຖານະ"], rows: [
        ...
        ["G-UX6 Final handoff", "final screens, responsive/state variants, assets, copy, annotations, open deviations", "Product Owner + Frontend Lead", "Admin Workflow R3.2 Application Launcher + single Header Sub-Menu + detailed workspace baseline ready; backend/auth/exhaustive states and final four-role sign-off pending"]
