...
const specs: Record<string, DocumentSpec> = {
  ...
  "full-ux-ui": {
    ...
    version: "0.10.1", status: "pending", statusLabel: "Guest Prototype R2.3 + Admin Workflow Prototype R4.1 ສຳເລັດ · ລໍ Backend/Auth/Failure-state/Manual Gates ກ່ອນ 1.0",
    approvalNote: "ສະບັບ 0.10.1 ປັບ Admin Workflow Prototype ເປັນ R4.1 ຫຼັງ Founder feedback ວ່າ UX ຈັດຕາມ system entity ຫຼາຍເກີນໄປ. Content Source Inbox ຈຶ່ງໃຊ້ task-first entry: ສະແດງ ‘ເພີ່ມຮ້ານ/ສະຖານທີ່’ ແລະ ‘ເພີ່ມວິດີໂອຣີວິວ’ ໃຫ້ເຫັນກ່ອນຕາຕະລາງ. Guided Flow ອະທິບາຍ Place ແລະ Review Source, ບັງຄັບໃຫ້ Source ເລືອກ Place, ກວດ Platform ຈາກ URL ແລະສະແດງ Place → Link → Rights → Publish. ການສ້າງ Place ຈະພາໄປ Place Draft; ການເພີ່ມ Source ຈະເຂົ້າ Source Inbox ພ້ອມ original link. Primary workspace data ຖືກຮັກສາໃນ browser session. 10 Modules / 55 views ແລະ seven workspace modes ຈາກ R4.0 ຍັງຄົງຢູ່. Data/Audit ຍັງເປັນ prototype fixtures; ຍັງບໍ່ມີ backend, authentication, durable persistence, real integrations ຫຼື production permission enforcement. UX-05 ຍັງບໍ່ແມ່ນ 1.0 ຈົນ TEC-01/02, exhaustive failure states, Manual Accessibility Gates ແລະ final four-role sign-off ສຳເລັດ.",
    ...
    sections: [
      { title: "Final Design Baseline R1 · ສິ່ງທີ່ສຳເລັດແລ້ວ", intro: "Final Design Gallery ແມ່ນ Web Documentation ສຳລັບກວດໜ້າຕາ, hierarchy, responsive direction ແລະ failure state. ມັນບໍ່ແມ່ນ Production application ແລະບໍ່ສ້າງ business logic ແຍກຈາກ PRO-02/04.", headers: ["Deliverable", "ຂອບເຂດ", "ວິທີກວດ", "ສະຖານະ"], rows: [
        ["Final Design Gallery 0.10.1", "SCR-G01/G02/G03/G05 ໃຊ້ Guest Prototype R2.3; SCR-A00—A09 embed Admin Workflow Prototype R4.1 ໂດຍກົງ", "Admin ກວດ Application Launcher → Module → Header Sub-Menu → task-first Content intake → functional workspace", "Interactive baseline ready; backend/auth pending"],
        ["Admin Workflow Prototype R4.1", "Standalone /admin-prototype ພ້ອມ 10 Main Modules, 55 views ແລະ guided Place/Review Source intake", "ລອງເພີ່ມ Place, ເພີ່ມ original video link, ເລືອກ Place, ກວດ Rights workflow ແລະ refresh session state", "Task-first interaction prototype ready"],
        ["Single-level Header Navigation", "Main Modules ບໍ່ສະແດງໃນ Header; Header Menu ສະແດງ Sub-Menu ຂອງ active Module ໂດຍກົງ", "ປ່ຽນ Module ຜ່ານ Application Launcher ແລ້ວກວດ Sub-Menu context ແລະ URL module/view", "R4.1 baseline ready"],
      ...
      { title: "Admin R4.1 Functional Coverage", intro: "Prototype ໃຊ້ task-first intake ແລະ workspace mode ທີ່ຕົງກັບວຽກແຕ່ລະປະເພດ ແທນການໃຊ້ table ຮູບແບບດຽວທຸກໜ້າ.", headers: ["Workspace mode", "ໃຊ້ກັບ", "Interaction ທີ່ທົດລອງໄດ້", "Boundary"], rows: [
      ...
      ], note: "R4.1 ເປັນ interaction/design evidence ສຳລັບພັດທະນາ; ບໍ່ແມ່ນ production system ແລະບໍ່ແທນ System Analysis/API/Auth specification."},
      ...
      { title: "Approval Gates ແລະ Sign-off", intro: "UX-05 1.0 ແມ່ນຄຳຢືນຢັນວ່າ package ພ້ອມໃຫ້ Developer ສ້າງໂດຍບໍ່ເດົາ. ການມີຮູບ Happy path ສວຍງາມບໍ່ພຽງພໍ.", headers: ["Gate", "ຕ້ອງມີ", "Signer", "ສະຖານະ"], rows: [
        ...
        ["G-UX6 Final handoff", "final screens, responsive/state variants, assets, copy, annotations, open deviations", "Product Owner + Frontend Lead", "Admin Workflow R4.1 task-first Content intake + 55 functional views ready; backend/auth/exhaustive states and final four-role sign-off pending"]
