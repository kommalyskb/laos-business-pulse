...
const specs: Record<string, DocumentSpec> = {
  ...
  "full-ux-ui": {
    ...
    version: "0.9.1", status: "pending", statusLabel: "Guest Prototype R2.3 + Admin Workflow Prototype R3.1 ສຳເລັດ · ລໍ Backend/Auth/Failure-state/Manual Gates ກ່ອນ 1.0",
    approvalNote: "ສະບັບ 0.9.1 ຂະຫຍາຍ standalone Admin Workflow Prototype ເປັນ R3.1. Prototype ຄຸ້ມ 10 Main Modules: Executive Overview, Operations Queue, Place 360, Content & Sources, Trust & Safety, Partner CRM, Sponsored Campaigns, Analytics, Revenue & Finance ແລະ System & Access. ແຕ່ລະ Main Module ມີ contextual Sub-Menu ຢູ່ header; Sub-Menu ມີໜ້າວຽກລະອຽດປະກອບດ້ວຍ KPI, filterable register, selected-record context, owner, due date, business control, required evidence, checklist, activity/audit ແລະ next-best action. Interaction ດັ້ງເດີມຂອງ R3.0 ເຊັ່ນ search, quick create, edit, workflow transition, publish, trust decision, finance verification, role/flag change ແລະ cross-module navigation ຍັງຄົງຢູ່. Data ແລະ Audit ເປັນ session fixtures ເທົ່ານັ້ນ; ຍັງບໍ່ມີ backend, authentication, durable persistence ຫຼື production permission enforcement. Odoo ໃຊ້ສະເພາະ information architecture/interaction anatomy; ສີ, Noto Sans Lao Variable, semantic states, border ແລະ focus style ໃຊ້ UX-04 Design Tokens. UX-05 ຍັງບໍ່ແມ່ນ 1.0 ຈົນ TEC-01/02, exhaustive failure states, Manual Accessibility Gates ແລະ final four-role sign-off ສຳເລັດ.",
    ...
    sections: [
      { title: "Final Design Baseline R1 · ສິ່ງທີ່ສຳເລັດແລ້ວ", intro: "Final Design Gallery ແມ່ນ Web Documentation ສຳລັບກວດໜ້າຕາ, hierarchy, responsive direction ແລະ failure state. ມັນບໍ່ແມ່ນ Production application ແລະບໍ່ສ້າງ business logic ແຍກຈາກ PRO-02/04.", headers: ["Deliverable", "ຂອບເຂດ", "ວິທີກວດ", "ສະຖານະ"], rows: [
        ["Final Design Gallery 0.9.1", "SCR-G01/G02/G03/G05 ໃຊ້ Guest Prototype R2.3; SCR-A00—A09 embed Admin Workflow Prototype R3.1 ໂດຍກົງ", "Guest ກວດຈາກ live source; Admin ກວດ 10 Main Modules, contextual Sub-Menu, detailed workspace, record control/evidence/action ແລະ audit", "Interactive baseline ready; backend/auth pending"],
        ["Admin Workflow Prototype R3.1", "Standalone /admin-prototype ພ້ອມ 10 Main Modules, 55 contextual Sub-Menu views, session fixtures ແລະ core operating workflows", "ລອງ Main Menu → Sub-Menu, select record, control/evidence checklist, assign/open/complete action ແລະ workflow ຂອງ default module view", "High-fidelity interaction prototype ready"],
        ["Contextual Sub-Navigation", "Overview, Operations, Places, Content, Trust, Partners, Campaigns, Analytics, Finance ແລະ System ມີ Sub-Menu ຢູ່ header ຂອງແຕ່ລະ Module", "ກົດ Main Module ແລ້ວກວດວ່າ Sub-Menu ປ່ຽນຕາມ context ແລະ URL ເກັບ module/view", "R3.1 baseline ready"],
      ...
      { title: "Approval Gates ແລະ Sign-off", intro: "UX-05 1.0 ແມ່ນຄຳຢືນຢັນວ່າ package ພ້ອມໃຫ້ Developer ສ້າງໂດຍບໍ່ເດົາ. ການມີຮູບ Happy path ສວຍງາມບໍ່ພຽງພໍ.", headers: ["Gate", "ຕ້ອງມີ", "Signer", "ສະຖານະ"], rows: [
        ...
        ["G-UX6 Final handoff", "final screens, responsive/state variants, assets, copy, annotations, open deviations", "Product Owner + Frontend Lead", "Admin Workflow R3.1 contextual navigation + detailed workspace baseline ready; backend/auth/exhaustive states and final four-role sign-off pending"]
