...
const reviews = [
  ["REV-01", "Delivery model", "ອະນຸມັດ 2 Tracks: Validation Pilot 6 ອາທິດກ່ອນ ແລະ Production MVP ເລີ່ມຫຼັງມີ approval/funding ຫຼືບໍ່?", "ອະນຸມັດ 2-track delivery; Prototype/Validation ບໍ່ແມ່ນ Production MVP."],
  ["REV-02", "Resourcing", "ອະນຸມັດ Founder-led hybrid build ພ້ອມ specialist checkpoint ແທນ full outsource ຫຼືບໍ່?", "ອະນຸມັດ Founder-led Hybrid; ຈ້າງ specialist ສະເພາະຈຸດຕາມຫຼັກຖານ ແລະບັນທຶກ founder person-days/shadow cost."],
  ["REV-03", "Scope control", "ອະນຸມັດ Must-only production baseline ແລະໃຫ້ Should ເຂົ້າຜ່ານ evidence/change control ຫຼືບໍ່?", "ອະນຸມັດ Must-only Production Baseline; Should ຕ້ອງຜ່ານ evidence, impact ແລະ Change Control."],
  ["REV-04", "Cost authority", "ອະນຸມັດວ່າ 25 ລ້ານກີບເປັນ Validation Budget; Production Build Budget ຕ້ອງມີ TEC-03/DEL-01 estimate ແລະ quote ໃໝ່ ຫຼືບໍ່?", "ອະນຸມັດການແຍກງົບ; Production Build Budget ຕ້ອງອະນຸມັດໃໝ່ຈາກ estimate ແລະ quote ທີ່ກວດໄດ້."],
  ["REV-05", "Effort & release", `ຮັບ ${totalMin}–${totalMax} person-days ເປັນ ROM ແລະບັງຄັບ P0→P4 release gates ຫຼືບໍ່?`, `ອະນຸມັດ ${totalMin}–${totalMax} person-days ເປັນ planning range ແລະ P0→P4 ເປັນ dependency/release gates; ຄຳນວນຄືນໃນ DEL-01.`],
] as const;
...
export default function TechnicalProposalDocument({ basePath }: { basePath: string }) {
  return <article className={`${styles.detailBody} ${styles.formalDocument}`}>
    ...
    <section className={styles.formalSection} id="tp-control"><h2><span>1.</span> ຂໍ້ມູນຄວບຄຸມເອກະສານ</h2><div className={styles.formalTableWrap}><table className={styles.formalTable}><tbody>
      <tr><th>ລະຫັດ / ສະບັບ</th><td>TEC-02 / 1.0</td><th>ສະຖານະ</th><td>ອະນຸມັດແລ້ວ</td></tr>
    ...
    </tbody></table></div><h3>1.1 ປະຫວັດ</h3><div className={styles.formalTableWrap}><table className={styles.formalTable}><thead><tr><th>ສະບັບ</th><th>ວັນທີ</th><th>ລາຍລະອຽດ</th></tr></thead><tbody><tr><td>0.1</td><td>28 ສິງຫາ 2026</td><td>ຮ່າງ delivery model, solution alternatives, work-package ROM, cost method, risk, release gate ແລະ 5 review decisions.</td></tr><tr><td>1.0</td><td>28 ສິງຫາ 2026</td><td>ອະນຸມັດ REV-01—05: 2-track delivery, Founder-led Hybrid, Must-only scope, budget separation ແລະ 75–110 person-day planning range ພ້ອມ P0—P4 gates.</td></tr></tbody></table></div></section>
    ...
    <section className={styles.formalSection} id="tp-review"><h2><span>11.</span> ບັນທຶກການອະນຸມັດ 5 ຂໍ້</h2><p>ການຕັດສິນ REV-01—05 ເປັນ Proposal Baseline ສຳລັບ TEC-03/04/06 ແລະ DEL-01. ການປ່ຽນ delivery model, scope, budget authority ຫຼື effort baseline ຕ້ອງມີ Change Decision.</p><div className={styles.formalTableWrap}><table className={styles.formalTable}><thead><tr><th>ID</th><th>ຫົວຂໍ້</th><th>ຄຳຖາມ</th><th>ຜົນຕັດສິນ</th><th>ສະຖານະ</th></tr></thead><tbody>{reviews.map(([id,title,question,recommendation])=><tr key={id}><td><code>{id}</code></td><td><strong>{title}</strong></td><td>{question}</td><td>{recommendation}</td><td><strong>ອະນຸມັດ</strong></td></tr>)}</tbody></table></div><div className={styles.formalApproval}><strong>TEC-02 · ສະບັບ 1.0</strong><p>Technical Proposal Baseline ຖືກອະນຸມັດ. 75–110 person-days ຍັງແມ່ນ planning range ບໍ່ແມ່ນ delivery commitment; Production Build Budget ແລະ calendar plan ຕ້ອງຖືກອະນຸມັດຕ່າງຫາກຫຼັງ TEC-03/04 ແລະ DEL-01.</p></div></section>
