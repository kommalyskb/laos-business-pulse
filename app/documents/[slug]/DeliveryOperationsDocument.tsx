...
const del01Reviews = [
  ["REV-01", "Calendar model", "75–110 person-days remains ROM; calendar dates are calculated only after weekly productive capacity, leave and support load are known.", "Approved · no Launch date commitment."],
  ["REV-02", "Team", "Founder-led Hybrid with independent QA and Security/Infrastructure checkpoints.", "Approved · names, availability and rates remain controlled inputs."],
  ...
  ["REV-05", "Budget authority", "Separate cash cost, Founder shadow cost, vendor quote, contingency and approved purchase.", "Approved structure · production amount/purchase authority remains pending."],
] as const;
...
const del05Reviews = [
  ...
  ["REV-02", "Segregation of duties", "Eight system roles stay separate. During solo-Founder Pilot one named person may hold multiple roles, with explicit action role, two-step state and self-review flag/audit.", "Approved · high-risk irreversible action still needs independent review."],
  ["REV-03", "Queue/SLA", "OPS-Q01—07 use the Pilot SLA table, severity/risk-first priority and documented pause/escalation.", "Approved · review actual performance after 30 operating days."],
  ["REV-04", "Maintenance cadence", "Daily/weekly/monthly/quarterly controls include a real monthly timed restore, not only backup-file presence.", "Approved"],
  ["REV-05", "Evidence/handover", "Every material action has case/task, reason, actor, timestamp, result and next owner; chat alone is not record.", "Approved"],
] as const;

function Header({ code, title, lao }: { code: string; title: string; lao: string }) {
  return <header className={styles.formalDocumentHeader}><p>{code} · DELIVERY &amp; OPERATIONS · 30 AUGUST 2026</p><h1>{title}</h1><h2>{lao}</h2><span className={`${styles.formalStatus} ${styles.formalDraftStatus}`}>0.2 · ມະຕິທົບທວນອະນຸມັດ · ລໍຖ້າຂໍ້ມູນປະຕິບັດກ່ອນ 1.0</span></header>;
}
...
function DevelopmentPlan({ basePath }: { basePath: string }) {
  ...
  return <article className={styles.formalDocument}><Header code="DEL-01" title="Development Plan" lao="ແຜນພັດທະນາຈາກ Approved Baseline ຫາ Public Pilot" />
    ...
    <section id="del01-review" className={styles.formalSection}><h2><span>07</span> ມະຕິທົບທວນ 5 ຂໍ້</h2><Reviews rows={del01Reviews} /><aside className={styles.formalDraftNotice}><strong>ຂໍ້ມູນທີ່ຍັງຕ້ອງປິດກ່ອນ 1.0</strong><p>ລະບຸ productive person-days ຕໍ່ອາທິດ, ຊື່/ຄວາມພ້ອມຂອງ QA ແລະ Security checkpoint, ອຳນາດໃຊ້ຈ່າຍ/ເພດານ Production ແລະລາຍການ P0 ທີ່ຈະເລີ່ມ. ການອະນຸມັດມະຕິຄັ້ງນີ້ຍັງບໍ່ສ້າງວັນ Launch ຫຼືອຳນາດຊື້.</p></aside></section>
...
function AdminOperations({ basePath }: { basePath: string }) {
  return <article className={styles.formalDocument}><Header code="DEL-05" title="Admin & Operations SOP" lao="ຄູ່ມືວຽກປະຈຳວັນ, Queue, Escalation ແລະ Handover" />
    ...
    <section className={styles.formalSection}><h2><span>08</span> ມະຕິທົບທວນ 5 ຂໍ້</h2><Reviews rows={del05Reviews} /><aside className={styles.formalDraftNotice}><strong>ກ່ອນ 1.0</strong><p>ລະບຸຊື່ຜູ້ຖື Role/backup checkpoint, ທົດລອງ Queue/SLA ດ້ວຍ fixtures, ເຮັດ monthly timed restore ແລະທົດລອງ handover/incident. ສະຖານະ “approved” ຂອງມະຕິບໍ່ແມ່ນຫຼັກຖານວ່າ controls ຖືກປະຕິບັດແລ້ວ.</p></aside></section>
