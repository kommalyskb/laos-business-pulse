...
export default function FinalDesignGallery({ basePath }: { basePath: string }) {
  ...
  return <main className={styles.site}>
    <header className={styles.topbar}><a href={`${basePath}/documents/full-ux-ui`}><b>UX-05</b><span>FINAL DESIGN GALLERY</span></a><nav><a href={`${basePath}/prototype`}>Guest Prototype R2.3</a><a href={`${basePath}/admin-prototype`}>Admin Prototype R4.0</a><a href={`${basePath}/design-system`}>UX-04 Gallery</a><a href={`${basePath}/documents`}>Documents</a></nav></header>
    <section className={styles.hero}><div><small>FULL UX/UI DESIGN · BASELINE 0.10.0</small><h1>ໜ້າຈໍສຳລັບຕັດສິນໃຈ<br/><em>ແລະພ້ອມທົດລອງ Workflow</em></h1></div><p>Guest/Pilot ໃຊ້ Prototype R2.3 ເປັນແຫຼ່ງອອກແບບດຽວ. Admin R4.0 ມີ 10 Main Modules ແລະ 55 functional views: ທຸກໜ້າລະບຸຈຸດປະສົງ, ຈຸດເລີ່ມ, completion condition, KPI, workflow, owner, evidence, checklist, action ແລະ session audit. Main Module ເລືອກຜ່ານ Application Launcher; Header ສະແດງສະເພາະ Sub-Menu ຂອງ Module ປັດຈຸບັນ.</p></section>
    <section className={styles.workspace}>
      ...
      <div className={styles.board}>
        <header className={styles.boardTools}><div><code>{screen.id}</code><span><b>{screen.name}</b><small>{screen.purpose}</small></span></div><div className={styles.switches}><span className={styles.sourceBadge}>{isGuest ? "LIVE · GUEST PROTOTYPE R2.3" : "LIVE · ADMIN WORKFLOW R4.0"}</span>{!isGuest ? <span aria-label="Viewport selector">{(["mobile", "tablet", "desktop"] as Viewport[]).map(item => <button key={item} aria-pressed={viewport === item} onClick={() => setViewport(item)}>{item}</button>)}</span> : <span aria-label="State selector">{screen.states.map(item => <button key={item} aria-pressed={uiState === item} onClick={() => setUiState(item)}>{stateLabels[item]}</button>)}</span>}</div></header>
        <div className={`${styles.canvas} ${styles[isGuest ? "mobile" : viewport]}`}><div className={styles.viewportLabel}><b>{isGuest ? "390 × 844 · Guest prototype" : viewport === "mobile" ? "390 × 844" : viewport === "tablet" ? "768 × 1024" : "1440 × 900"}</b><span>{isGuest ? uiState : "interactive"}</span></div><div className={`${styles.preview} ${styles.prototypePreview}`}>{isGuest ? <iframe key={guestPrototypeUrl} src={guestPrototypeUrl} title={`Prototype R2.3 · ${screen.name} · ${stateLabels[uiState]}`} /> : <iframe key={adminPrototypeUrl} src={adminPrototypeUrl} title={`Admin Workflow Prototype R4.0 · ${screen.name}`} />}</div></div>
        <footer className={styles.specBar}><div><small>PRIMARY OUTCOME</small><b>{screen.purpose}</b></div><div><small>DATA CONTRACT</small><b>{screen.group === "Admin" ? "Application Launcher · contextual Header Sub-Menu · 7 workspace modes · session audit" : "Place ID · source · freshness · action availability"}</b></div><div><small>HANDOFF STATUS</small><b>{isGuest ? "Prototype parity · single source of truth" : "Admin Workflow R4.0 · 55 functional views · backend/auth not connected"}</b></div></footer>
      </div>
    </section>
  </main>;
}
